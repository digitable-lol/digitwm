 ...
 *
 * ПОЧЕМУ ЭТОТ ХАРНЕСС ЕЩЁ НА NODE, КОГДА ДЕСЯТЬ СПЕК УЖЕ НА FLANG.
 *
 * Не потому, что руки не дошли. Спеки в `fts/flang/` - политики: вход из
 * нескольких чисел, выход одно число, и обещание говорит о РЕЗУЛЬТАТЕ одного
 * вызова. Оба обещания здесь говорят о другом - об ОТНОШЕНИИ ДВУХ СОСТОЯНИЙ
 * ленты, до вставки окна и после неё, - и такого предмета среди десяти спек
 * нет вовсе. Переезд начинается не с харнесса, а со спеки: пока `ribbon_insert()`
 * не описан как функция «лента -> лента», обещанию «соседей не сжали» не в чем
 * жить. (Выразить его flang умеет: `обеспечивает` видит и параметры, и
 * результат. Не описано - значит не описано, а не «нельзя».)
 *
 * Второе, что держит: корпус. Триста двадцать сценариев берутся засеянным
 * генератором на 32-битной арифметике JS (`Math.imul`, сдвиги, исключающее
 * или). Во flang нет ни одной побитовой формы - в `stdlib/sha256.flang` их
 * складывают из арифметики вручную, - и тот же корпус пришлось бы собирать
 * заново, то есть сказать «те же 320, что были» стало бы нельзя.
 *
 * ЧТО ДОЛЖНО СЛУЧИТЬСЯ, ЧТОБЫ ЭТОТ ФАЙЛ УШЁЛ: спека ленты как значения (а не
 * десять спек её чисел) и корпус, который выражается без побитовой арифметики
 * - или выражается ею, если во flang она появится.
 *
 * Запуск:
 ...
 * свойством модели (fts/flang/window-height.flang, «высота не меньше минимальной»), и
