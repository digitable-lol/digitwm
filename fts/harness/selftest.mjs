...
const MUTATIONS = [
	...
	{
		/*
		 * Вторая ось портится своей мутацией, а не мутацией первой: обе
		 * модели написаны одними словами, и мутация в «scroll-offset» не
		 * доказывает, что векторы «stack-offset» вообще на что-то смотрят.
		 */
		title: "поле у верхней кромки становится вдвое уже зазора",
		utility: "stack-offset",
		edits: [
			["stack-offset.fts", "то добавить -100 процентов от поля зазор", "то добавить -50 процентов от поля зазор"],
			["stack-offset.en.fts", "then add -100 percent of field gap", "then add -50 percent of field gap"],
		],
	},
	{
	...
	{
		/*
		 * Спор двух панелей выигрывает ДАЛЬНЯЯ, а не ближняя.  Мутация
		 * подменяет ровно решение, а не арифметику: обе панели по-прежнему
		 * получают ровно область на двоих, и ни одно свойство модели -
		 * «не меньше нуля», «не больше области» - при этом не нарушается.
		 * Поймать такое можно только вектором, и потому мутация здесь: без
		 * пары векторов на одну и ту же область с разным «спрашивают про
		 * дальнюю» перемена победителя прошла бы молча, а строка состояния
		 * внизу экрана уехала бы под кромку.
		 */
		title: "спор двух панелей выигрывает дальняя, а не ближняя",
		utility: "strut-pair",
		edits: [
			["strut-pair.fts", "правило «Спор: дальней остаётся то, что не взяла ближняя»\n      если «перебор» больше 0\n      и «спрашивают про дальнюю» равен да\n      то результат равен 100 процентов от поля «остаток дальнему»", "правило «Спор: дальней остаётся то, что не взяла ближняя»\n      если «перебор» больше 0\n      и «спрашивают про дальнюю» равен да\n      то результат равен 100 процентов от поля «дальняя полоса»"],
			["strut-pair.en.fts", "rule \"Argument: the far edge gets what the near one did not take\"\n      if overrun is greater than 0\n      and want-far equals true\n      then result equals 100 percent of field far-left", "rule \"Argument: the far edge gets what the near one did not take\"\n      if overrun is greater than 0\n      and want-far equals true\n      then result equals 100 percent of field far-strut"],
		],
	},
	{
