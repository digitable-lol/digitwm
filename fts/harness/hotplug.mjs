 ...
 * сказано прямо.
 *
 * ПОЧЕМУ ЭТОТ ХАРНЕСС ЕЩЁ НА NODE. Причина та же, что у `invariants.mjs`, и
 * даже резче: здесь сверяются ТРИ состояния СРАЗУ НЕСКОЛЬКИХ лент - до
 * события RandR, после него и после возврата монитора. Ни одна из десяти спек
 * не описывает ни ленту как значение, ни набор лент по выходам, а обещания
 * DGT-WM-08 - «колонки не потерялись», «конфигурация восстановилась» - только
 * об этом и говорят. Сверять их со спекой пока не с чем: спеки нет.
 *
 * Отсюда и разбор вывода в `probe.mjs`: сверять два потока байт, как это
 * делает `fts/flang/layout.flang`, здесь нечем - печатать эталон не из чего.
 * Читается то, что напечатал probe, и проверяется само с собой.
 *
 * ЧТО ДОЛЖНО СЛУЧИТЬСЯ, ЧТОБЫ ЭТОТ ФАЙЛ УШЁЛ: спека «набор выходов -> набор
 * лент» на flang. Тогда сценарии ниже станут её примерами, а сверка с живым
 * менеджером - планом `flang io`, как уже сделано для раскладки.
 *
 * Запуск:
