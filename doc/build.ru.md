...
не `digitwm`: так конфигурация, написанная для cwm, и всё написанное про cwm
переносится без правок — чего это стоит, разобрано в
[pkgsrc/README.ru.md](../pkgsrc/README.ru.md). Сам файл настроек — наш; порядок
поиска ниже.
...
запускает `cwm -c` с ней). Запущенный сам по себе `cwm` берёт первый файл из
существующих:
...
| 4 | `~/.cwmrc` | имя cwm: читается, чтобы пришедший из cwm продолжал работать, — и об этом говорится вслух, один раз, в stderr |

`cwm -n` печатает выбранный файл и останавливается. Порядок написан один раз —
`confpath.c`, — и его же компилирует сборка macOS ([macos.ru.md](macos.ru.md));
`tools/check-config-order.sh` задаёт обоим двоичным файлам одни и те же семь
вопросов в CI, а `--selfcheck` показывает, как сторож краснеет на нарушенном
порядке. Все настройки описаны в `cwmrc(5)`; собственные настройки ленты собраны
в [ribbon.ru.md](ribbon.ru.md).
