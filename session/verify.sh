...
#
# Повторная установка входит в четвёртый вопрос наравне с чужими файлами:
# установщик, не узнающий свой собственный init.lua, кладёт рядом второй — и
# у человека оказываются две конфигурации Neovim вместо одной. Спрашивается и
# это: метка в нашем файле, отсутствие удвоения, строка панели в автозапуске.
#
...
# Решение про панель одно и записано в session/README.md: панель ставится, но
# при входе не поднимается — её поднимает Mod4+Shift+b. Обещание «строка для
# неё есть в автозапуске, раскомментируйте» проверяемо: строки может не быть
# вовсе, и тогда обещание ложное. Автозапуск — файл человека, поэтому
# раскомментированная строка не ошибка, а его выбор; проверка говорит, какой
# именно из двух вариантов на этой машине, и молчать не будет ни в одном.
if [ -f "$CONFIG_DIR/autostart" ]; then
  if grep -qE '^[[:space:]]*#.*digitwm-panel' "$CONFIG_DIR/autostart"; then
    ok "автозапуск: строка панели закомментирована — при входе панель не поднимается"
  elif grep -q 'digitwm-panel' "$CONFIG_DIR/autostart"; then
    ok "автозапуск: строка панели раскомментирована — панель поднимется при входе"
  else
    bad "автозапуск" "нет строки digitwm-panel: включить панель при входе нечем"
  fi
fi
...
# Метка из шаблона session/config/nvim-init.lua. По ней установщик узнаёт свой
# файл на повторном запуске; без неё он снова сочтёт его вашим.
...
if [ -z "$nvim_ours" ]; then
  ...
  expect_reference "$nvim_ours" "$NVIM_MARKER" \
    "конфигурация Neovim помечена как наша — повторная установка её узнает"
fi

# Удвоения быть не должно. digitwm-session.lua кладётся рядом ТОЛЬКО с чужим
# init.lua; рядом с нашим же — это след повторной установки, которая не
# узнала собственный файл и положила второй с тем же содержимым.
if [ -f "$nvim_init" ] && [ -f "$nvim_side" ] && grep -qF "$NVIM_MARKER" "$nvim_init"; then
  bad "конфигурация Neovim" "рядом с нашим же init.lua лежит лишний $nvim_side"
else
  ok "конфигурация Neovim не удвоена"
fi
