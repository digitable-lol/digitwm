...
# Свой каталог, а не угаданное имя в общем /tmp.
#
# Было `${TMPDIR:-/tmp}/digitwm-nox.$$` плюс `mkdir -p`. Пара «предсказуемое
# имя + mkdir -p» означает ровно одно: если каталог с таким именем уже есть,
# скрипт молча берёт ЧУЖОЙ каталог за свой - а `cleanup` в конце стирает его
# целиком вместе с содержимым. Номер процесса предсказуем и переиспользуется
# (после перезагрузки, при обёртке PID и в чужом namespace он совпадает
# запросто), и /tmp здесь общий на всех: сторож, роняющий чужой файл, не
# сторож. Проверено: подложенный `digitwm-nox.<pid>/PRECIOUS/keep.txt`
# исчезал после прогона.
#
# `mktemp -d` снимает оба конца сразу: имя выдаёт ядро, а не арифметика по
# PID, каталог создаётся с правами 0700 и существовать заранее не может - две
# копии скрипта не встретятся, даже если запущены в одну миллисекунду. TMPDIR
# по-прежнему уважается: вынести черновик из /tmp - дело вызывающего.
work=
cleanup() { if [ -n "$work" ]; then rm -rf "$work"; fi; }
trap cleanup EXIT INT TERM

work=$(mktemp -d "${TMPDIR:-/tmp}/digitwm-nox.XXXXXXXX") || {
