...
#
# Второй проход отделяет девять политик - чистую арифметику - в собственную
...
[ "$nfn" -eq 9 ] || {
	echo "выкушено $nfn политик вместо девяти - изменился набор ribbon_policy_*" >&2
	exit 1
}

printf 'девять политик отдельной единицей: '
