...
/*
 * Напечатанная из flang арифметика зовёт libc по обычным именам, а не по
 * нашим x*(). Та же куча, тот же ответ на нехватку: сказать, а не испортить
 * соседа. Арена рантайма берёт память один раз на вызов политики и
 * освобождает её тем же `free`, который здесь ничего не делает, - куча растёт
 * на страницу, живущую минуты, и это уже сосчитано в DGT_HEAP.
 */
void *
malloc(size_t size)
{
...
/*
 * FLANG_TIME_OUT и подобные: среды здесь нет вовсе, и ответ «ничего не
 * задано» - не заглушка, а правда. Именно он выключает единственный кусок
 * рантайма, которому нужен POSIX.
 */
char *
...
/*
 * Обёртка политики говорит сюда, если напечатанное отказалось отвечать
 * числом (ribbon.c, ribbon_flang_int). За 526 871 вход сверки этого не
 * случилось ни разу; здесь оно СЧИТАЕТСЯ, а не печатается, по двум причинам:
 * stdio в этом модуле нет вовсе, и молчаливый отказ был бы хуже громкого -
 * страница и сверка могут спросить счётчик и покраснеть.
 */
static unsigned long	 dgt_refusals;
...
/*
 * Единственное, что рантайм печати просит у stdio на этом пути: сборка текста
 * беды. НАСТОЯЩЕГО printf здесь нет и не будет - это тысяча строк ради строки,
 * которую некому прочитать, - поэтому сюда кладётся формат как есть, без
 * подстановки доводов, и возвращается его длина.
 *
 * Чем это можно, а чем нельзя пользоваться. Текст беды рождается ТОЛЬКО когда
 * политика отказалась отвечать числом; на пути успеха его не строит никто, и
 * ни один из 526 871 входа сверки (tools/check-ribbon-flang.sh) его не
 * потребовал. Число отказов при этом считается честно - dgt_refused(), - так
 * что «сколько раз» страница узнает точно, а «почему» на ней будет неполным.
 * Признать это вслух дешевле, чем писать здесь второй printf и ошибиться в нём.
 */
int
...
/*
 * Три функции из libm, которых просит рантайм печати.  floor и fabs у wasm
 * есть инструкциями, и встроенные компилятора в них и превращаются; fmod
 * инструкции не имеет, и потому написан.
 *
 * ПОЧЕМУ fmod ВАЖЕН, а не просто нужен для сборки.  Он стоит в постусловии
 * «Деления нацело» - того самого, которое считает ширину колонки: частное и
 * остаток обязаны собрать делимое обратно.  Ошибись он на единицу, ширины
 * разъедутся, и разъедутся ЗАМЕТНО: эта же страница сверяется с двоичным
 * файлом на 500 случайных случаях, и ширина стоит в каждом из них.
 *
 * Деление double на double округляется, поэтому trunc(x / y) может
 * промахнуться на единицу в обе стороны.  Двух поправок довольно: одна
 * возвращает остаток внутрь |y|, вторая - к знаку делимого, как того требует
 * C99.
 */
double
floor(double x)
...
fmod(double x, double y)
{
	...
	ay = __builtin_fabs(y);
	q = __builtin_trunc(x / y);
	r = x - (q * y);

	if (__builtin_fabs(r) >= ay)
		r -= (r < 0.0) ? -ay : ay;
	if (r != 0.0 && ((r < 0.0) != (x < 0.0)))
		r += (x < 0.0) ? -ay : ay;
...
/*
 * Правила вставки задаёт cwmrc, а у страницы его нет: разбирать нечего,
 * значит и совпасть не с чем.  RIBBON_RULE_NONE - не заглушка ради
 * компоновки, а верный ответ для поверхности без конфигурации: лента
 * ведёт себя ровно так, как на машине, где ни одной строки ribbonrule
 * не написано.  Появится у страницы конфигурация - здесь появится и
 * разбор, а не второе умолчание.
 */
int
